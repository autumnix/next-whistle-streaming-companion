...
from nwsc.db.repository import LAST_PLAYED_KV, Repository
...
class TestGameOperations:
    async def test_create_game(self, repo: Repository):
        await repo.create_game("game-1")
        game_id = await repo.get_current_game_id()
        assert game_id == "game-1"

    async def test_create_game_sets_current(self, repo: Repository):
        await repo.create_game("game-1")
        await repo.create_game("game-2")
        assert await repo.get_current_game_id() == "game-2"

    async def test_create_game_clears_last_played(self, repo: Repository):
        await repo.create_game("game-1")
        async with repo._db.connection() as conn:
            val = await repo.kv_get(conn, LAST_PLAYED_KV)
            assert val == ""

    async def test_get_game(self, repo: Repository):
        await repo.create_game("game-1")
